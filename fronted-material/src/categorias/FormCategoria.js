...
export const FormCategoria= (props) => {
  ...
  const cargarCategoria = (id) => {
    axios.get("http://localhost:8000/api/categoria/" + id, { headers }).then(
      (response) => {
        if (response.data.res !== "success") {
          swal(
            "Error ❌!",
            "No se ha podido obtener la lista de Categorias",
            "danger"
          );
          return;
        }
        ...
        setAbbreviation(categoria.abbreviation);
      },
      (error) => {
  ...
  const enviarDatos = (e) => {
    ...
    if (id == 0) {
      enviarInsertar(categoria);
    } else {
      enviarActualizar(categoria, id);
    }
  };
  const enviarInsertar = (categoria) => {
    axios
      .post("http://localhost:8000/api/categoria/", categoria, { headers })
      .then(
        (response) => {
          if (response.data.res !== "success") {
            swal(
          ...
          swal(
            "Buen Trabajo!",
            "El nuevo categoria fue añadido Correctamente!",
            "success"
          );
  ...
  const enviarActualizar = (categoria, id) => {
    axios
      .put("http://localhost:8000/api/categoria/" + id, categoria, { headers })
      .then(
        (response) => {
          if (response.data.res !== "success") {
          ...
          swal(
            "Buen Trabajo!",
            "El categoria fue actualizada Correctamente!",
            "success"
          );
          history.push("/categorias");
        },
            ...
            <form onSubmit={enviarDatos}>
              ...
              <MiColumnaSelect>
                <MiGroup>
                  <MiInput
                    type="text"
                    placeholder="Escribe el Categoria"
                    required=""
