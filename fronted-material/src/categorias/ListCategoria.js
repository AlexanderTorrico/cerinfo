...
export const ListCategoria = (props) => {
 ...
 const access_token = localStorage.getItem("access_token");
  ...
  const traerCategorias = () => {
    axios
      .get("http://localhost:8000/api/categoria", { headers })
      .then((response) => {
  ...
  const eliminarCategoria = (id) => {
    swal({
      ...
      (willDelete) => {
        if (willDelete) {
          axios.delete("http://localhost:8000/api/categoria/" + id, {
            headers,
          });
          swal("Poof! El Categoria  fue eliminado correctamente!", {
            icon: "success",
          });
          traerCategorias();
        } else {
          swal("Ha cancelado la eliminacion del Categoria!");
        }
      },
      (error) => {
        if (error.response.status === 401) {
    ...
    <>
      <div className="mt-3 text-center">
        ...
        <div className="container-fluid">
          <h2 className="text-center all-tittles">Lista de Categoria</h2>
        </div>
        <div className="container-fluid">
          <Link
