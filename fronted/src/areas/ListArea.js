...
export const ListArea = (props) => {
 ...
 const access_token = localStorage.getItem("access_token");
  ...
  const traerAreas = () => {
    axios
      .get("http://localhost:8000/api/area/", { headers })
      .then((response) => {
        console.log(response.data);
        setListaAreas(response.data.data);
      })
      .catch((error) => {
  ...
  const eliminarAreas = (id) => {
    swal({
      ...
      (willDelete) => {
        if (willDelete) {
          axios.delete("http://localhost:8000/api/area/" + id, {
            headers,
          });
          swal("Poof! El Area  fue eliminado correctamente!", {
            icon: "success",
          });
          traerAreas();
        } else {
          swal("Ha cancelado la eliminacion del Area!");
        }
      },
      (error) => {
        if (error.response.status === 401) {
    ...
    <>
      <div className="mt-3 text-center">
        ...
        <div className="container-fluid">
          <h2 className="text-center all-tittles">Lista de Areas</h2>
        </div>
        <div className="container-fluid">
          <Link
