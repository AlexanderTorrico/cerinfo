...
export const FormGenero= (props) => {
  ...
  const cargarGenero = (id) => {
    axios.get("http://localhost:8000/api/gender/" + id, { headers }).then(
      (response) => {
        if (response.data.res !== "success") {
          swal(
            "Error ❌!",
            "No se ha podido obtener la lista de Genero",
            "danger"
          );
          return;
        }
  ...
  const enviarDatos = (e) => {
    ...
    if (id == 0) {
      enviarInsertar(genero);
    } else {
      enviarActualizar(genero, id);
    }
  };
  const enviarInsertar = (genero) => {
    axios
  ...
  const enviarActualizar = (genero, id) => {
    axios
      .put("http://localhost:8000/api/gender/" + id, genero, { headers })
      .then(
        (response) => {
          if (response.data.res !== "success") {
