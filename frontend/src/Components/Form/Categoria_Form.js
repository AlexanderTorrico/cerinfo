...
class Categoria_Form extends Component {
    constructor(props) {
        super(props);
    ...
    peticionPost = () => {
        axios.post(this.props.categoriaUrl, this.state.inputs).then(res=>{
            console.log()
            this.closeDrawer()
        }).catch(error=>{
    ...
    peticionPut = () => {
        axios.put(this.props.categoriaUrl+this.state.inputs.id, this.state.inputs).then(res=>{
            this.closeDrawer()
        }).catch(error=>{
            console.log(error.message)
        })
    ...
    render() {
        return (
            <React.Fragment>
                    ...
                    <DrawerContent>
                        <DrawerHeader>
                            {this.props.drawerMode==='add'? <h1>Añadir Categoria</h1> : <h1>Editar Categoria</h1> }
                            <br />
                            <Divider />
                        </DrawerHeader>
