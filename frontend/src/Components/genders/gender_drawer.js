...
class GenderDrawer extends Component {
    constructor(props) {
        super(props);
    ...
    peticionPost = () => {
        axios.post(this.props.gendersUrl, this.state.inputs).then(res=>{
            this.closeDrawer()
        }).catch(error=>{
    ...
    peticionPut = () => {
        axios.put(this.props.gendersUrl+this.state.inputs.id, this.state.inputs).then(res=>{
            this.closeDrawer()
        }).catch(error=>{
            console.log(error.message)
        })
    ...
    render() {
        return (
            <React.Fragment>
                    ...
                    <DrawerContent>
                        <DrawerHeader>
                            {this.props.drawerMode==='add'? <h1>Añadir Genero</h1> : <h1>Editar Genero</h1> }
                            <br />
                            <Divider />
                        </DrawerHeader>
